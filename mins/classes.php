		...
		foreach ($reader->getSheetIterator() as $sheet) 
		{
			...
			foreach ($sheet->getRowIterator() as $row) 
		    {
		    	...
		    	if($fila>=$filaInicial )
		    	{
			        ...
			        if(trim($registro[0])!="")
			        {
				       	$sqlInsert="INSERT INTO registrosMinistraciones (ministracionesTemporales_idministracionesTemporales, folioIdentificador, nombreAhorrador, representanteAlbacea, parteSocial, cuentasAhorro, cuentasInversion, depositosGarantia, chequesNoCobrados, otrosDepositos, prestamosCargo, saldoTotal, montoMinistrar ) VALUES ('".$idministracionesTemporales."', '".$registro[1]."', '".$registro[2]."', '".$registro[3]."', '".$registro[4]."', '".$registro[5]."', '".$registro[6]."', '".$registro[7]."', '".$registro[8]."', '".$registro[9]."', '".$registro[10]."', '".$registro[11]."', '".$registro[12]."') ";
